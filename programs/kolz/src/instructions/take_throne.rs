...
pub struct TakeThrone<'info> {
    ...
    #[account(
        constraint = pump_mint.key() == launch.pump_mint @ KolzError::MintMismatch,
    )]
    pub pump_mint: Account<'info, Mint>,

    /// Challenger's pump token ATA. Used to read their balance.
    #[account(
        constraint = challenger_pump_ata.owner == challenger.key() @ KolzError::AtaOwnerMismatch,
        constraint = challenger_pump_ata.mint == pump_mint.key() @ KolzError::MintMismatch,
    )]
    pub challenger_pump_ata: Account<'info, TokenAccount>,

    /// The NFT mint.
    #[account(
        constraint = nft_mint.key() == king.nft_mint @ KolzError::MintMismatch,
    )]
    ...
    #[account(
        mut,
        constraint = escrow_vault.key() == king.nft_escrow_vault @ KolzError::MintMismatch,
    )]
    ...
    #[account(
        mut,
        constraint = challenger_nft_ata.owner == challenger.key() @ KolzError::AtaOwnerMismatch,
        constraint = challenger_nft_ata.mint == nft_mint.key() @ KolzError::MintMismatch,
    )]
...
pub fn handler(ctx: Context<TakeThrone>) -> Result<()> {
    ...
    if king.settles_at_slot != 0 && now >= king.settles_at_slot {
        return err!(KolzError::SettlementPeriodEnded);
    }

    let challenger_balance = ctx.accounts.challenger_pump_ata.amount;
    require!(
        challenger_balance > king.champion_balance,
        KolzError::NotTopHolder
    );
    require!(challenger_balance > 0, KolzError::InvalidAmount);
    ...
    if first_capture {
        king.settles_at_slot = now
            ...
            .ok_or(error!(KolzError::SettlementNotReady))?;
    }
    ...
    msg!(
        "kolz: throne taken pet={} new_champion={} balance={} take_overs={}",
        pet_key,
        king.current_champion,
